...
export function EditProduceScreen({
  listing,
  onBack,
  onSubmit,
}: {
  ...
  const { t, tCrop } = useT();
  const [crop, setCrop] = useState(cropOptions.find((c) => c.name === listing.crop) || cropOptions[0]);
  const [qty, setQty] = useState(listing.quantity.toString());
  const [unit, setUnit] = useState(listing.unit || "quintal");
  const [readyDate, setReadyDate] = useState(listing.readyDate);
  ...
  if (submitted) {
    return (
      <div className="flex min-h-screen flex-col bg-slate-50/50">
        <AppHeader title="Listing Updated" onBack={onBack} />
        <div className="flex flex-1 flex-col items-center justify-center px-6 py-10 text-center animate-fade-in">
          ...
          <h2 className="mt-8 text-2xl font-black tracking-tight text-slate-900">Changes Saved!</h2>
          <p className="mt-3 max-w-xs text-[15px] font-medium text-slate-500 leading-relaxed">Your specific listing updates have been pushed to the network.</p>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50/50 pb-10">
      <AppHeader title="Edit Listing" subtitle={`Update details for #${listing.id}`} onBack={onBack} />
